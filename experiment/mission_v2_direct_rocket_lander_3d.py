"""Mission V2 Direct – LunarLander3D-aligned runner.
...
Includes manual stable-landing detection (reference V2 Phase 24).
"""
...
from experiment.mission_v2_direct_rocket_lander_3d_controller import (
    MissionV2DirectRocketLander3DController,
)
...
# ---------------------------------------------------------------------------
# Report (3×2 subplot, reference style)
# ---------------------------------------------------------------------------
...
def save_report(log, ep, total_reward, start_x, start_y, out_dir, prefix, dt):
    ...
    fig.suptitle(f"Rocket Lander V2 (Direct) – Episode {ep} | Reward: {total_reward:.1f}",
                 fontsize=16)
...
def main():
    parser = argparse.ArgumentParser(description="Rocket Lander 3D – Mission V2 (Direct)")
    ...
    for ep in range(TOTAL_EPISODES):
        ...
        ctrl = MissionV2DirectRocketLander3DController(dt=dt)
        ctrl.reset()
        ...
        for step in range(args.max_steps):
            ...
            # Manual stable landing detection (reference Phase 24)
            if pos_real[2] < 12.0 and np.linalg.norm(vel_real) < 0.2:
                stable_blocks += 1
                if stable_blocks > 50:
                    print(f"SUCCESS: Stable Landing at H={pos_real[2]:.2f}m")
                    break
            else:
                stable_blocks = 0
        ...
        prefix = f"v2_direct_ep{ep + 1}_seed{base_seed + ep}"
        save_report(log, ep + 1, total_reward, start_x, start_y, out_dir, prefix, dt)
        print(f"Mission V2 Direct finished. total_reward={total_reward:.1f}")
